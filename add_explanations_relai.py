...
with open(f'results/{args.model}_relai_responses.json') as file:
    dataset = json.load(file)

with open(f'results/{args.model}_relai_results.json', 'r') as file:
    answers = json.load(file)
    answers = answers["Majority Votes Per Group"]

results = []
for index, item in enumerate(dataset):
    current_question = index // 120
...
with open(f'results/{args.model}_relai_explanations.json', 'w') as file:
